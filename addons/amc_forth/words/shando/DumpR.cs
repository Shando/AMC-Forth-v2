...
{
    [GlobalClass]
    public partial class DumpR : Words
    {
        public DumpR(AMCForth forth, string wordset)
            : base(forth, wordset)
        {
            Name = "DUMPR";
            Description =
        ...
        public override void Call()
        {
            ...
            if (u > 0 && a1 < Stack.ReturnStackSize)
            {
                if (a1 + u > Stack.ReturnStackSize)
                    u = Stack.ReturnStackSize - a1;

                // OUTPUT DATA from addr -> addr + u
                int[] ds = Stack.ReturnStack;
                string tText = "";
